...
static struct queue_entry* top_cb_rated[MAP_SIZE];
static u8 cb_score_changed;
...
static u64 condition_values[2*MAP_SIZE];
static u32* critical_ids;
static u32 critical_count[MAP_SIZE];
static u32* target_count;


static u8 is_target;
static u8 is_rare_target;
...
void alloca_cond_bits(struct queue_entry* q) {
  ...
  q->cond_bits = ck_alloc(sizeof(u32)*100);
  q->cond_vals = ck_alloc(sizeof(u64)*200);
...
static void update_cb_bitmap_score(struct queue_entry* q) {
  ...
  for (i = 0; i < q->critical_bbs[0]; i++) {
    ...
    if (top_cb_rated[id]) {
      if (fav_distance > top_cb_rated[id]->distance) continue;

      if (fav_distance == top_cb_rated[id]->distance) {
        if (fav_factor > top_cb_rated[id]->exec_us * top_cb_rated[id]->len)
          continue;
      }  
    }
...
static void cb_cull_queue(void) {
  struct queue_entry* q;

  u32 i;

  if (dumb_mode || !cb_score_changed) return;
  ...
  for (i = 0; i < MAP_SIZE; i++)
    if (top_cb_rated[i] && !solved_cbbs[i] && (critical_count[i] < BLOCK_TIMES)) {
...
EXP_ST void setup_shm(void) {
  ...
  if (!dumb_mode) setenv("__CRITICAL_BB_ID", shm_str2, 1);
  ...
  // distace bb trace
  s32 shm_id3 = shmget(IPC_PRIVATE, MAP_SIZE, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_id3 < 0) PFATAL("shmget() failed");

  u8* shm_str3;

  shm_str3 = alloc_printf("%d", shm_id3);

  if (!dumb_mode) setenv("__DISTANCE_BB_ID", shm_str3, 1);

  ck_free(shm_str3);

  distance_bits = shmat(shm_id3, NULL, 0);

  if (!distance_bits) PFATAL("shmat() failed");

  // condition trace

  s32 shm_id4 = shmget(IPC_PRIVATE, sizeof(u64)*cond_num*2, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_id4 < 0) PFATAL("shmget() failed");

  u8* shm_str4;

  shm_str4 = alloc_printf("%d", shm_id4);

  if (!dumb_mode) setenv(CONDITION_VAR, shm_str4, 1);

  ck_free(shm_str4);

  condition_bits = shmat(shm_id4, NULL, 0);

  if (!condition_bits) PFATAL("shmat() failed");

  s32 shm_id5 = shmget(IPC_PRIVATE, cond_num, IPC_CREAT | IPC_EXCL | 0600);

  if (shm_id5 < 0) PFATAL("shmget() failed");

  u8* shm_str5;

  shm_str5 = alloc_printf("%d", shm_id5);

  if (!dumb_mode) setenv("__COND_SHM_ID", shm_str5, 1);

  ck_free(shm_str5);

  cond_bits = shmat(shm_id5, NULL, 0);

  if (!cond_bits) PFATAL("shmat() failed");
}
...
EXP_ST void init_forkserver2(char** argv) {
  ...
  if (forksrv_pid2 < 0) PFATAL("fork() failed");
  ...
  if (!forksrv_pid2) {
    ...
    if (!getrlimit(RLIMIT_NOFILE, &r) && r.rlim_cur < FORKSRV_FD2 + 2) {

      r.rlim_cur = FORKSRV_FD2 + 2;
      setrlimit(RLIMIT_NOFILE, &r); /* Ignore errors */
    ...
    if (dup2(ctl_pipe[0], FORKSRV_FD2) < 0) PFATAL("dup2() failed");
    if (dup2(st_pipe[1], FORKSRV_FD2 + 1) < 0) PFATAL("dup2() failed");
  ...
  fsrv_ctl_fd2 = ctl_pipe[1];
  fsrv_st_fd2  = st_pipe[0];
  ...
  rlen = read(fsrv_st_fd2, &status, 4);
  ...
  if (waitpid(forksrv_pid2, &status, 0) <= 0)
    PFATAL("waitpid() failed");
...
static u8 run_target2(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (child_pid2 < 0) PFATAL("fork() failed");
    ...
    if ((res = write(fsrv_ctl_fd2, &prev_timed_out, 4)) != 4) {
    ...
    if ((res = read(fsrv_st_fd2, &child_pid2, 4)) != 4) {
    ...
    if (child_pid2 <= 0) FATAL("Fork server is misbehaving (OOM?)");
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (waitpid(child_pid2, &status, 0) <= 0) PFATAL("waitpid() failed");

  } else {

    s32 res;

    if ((res = read(fsrv_st_fd2, &status, 4)) != 4) {
  ...
  if (!WIFSTOPPED(status)) child_pid2 = 0;
...
static void cb_count(void) {
  ...
  for (i=0;i<critical_ids[0];i++) {
    if (unlikely(critical_bits[critical_ids[i+1]] == 1)) {
      if (unlikely(critical_count[critical_ids[i+1]] > BIG_NUMBER))
        continue;
      if (critical_count[critical_ids[i+1]] < BLOCK_TIMES)
...
static void cond_stat(void) {
  u32 i;
  for (i=1;i<cond_num;i++) {
    if (cond_bits[i]) {
      solved_cond[i] |= cond_bits[i];
    }
  }
}

static u32 num_equal_bits(u64 l, u64 r) {
  u32 count = 0;
  u64 res = ~(l ^ r);
  while (res != 0) {
    count += res & 1;
...
keep_as_crash:
      ...
      if (is_target) {
        unique_crashes++;
        fprintf(crashes_log, "%llu : %s --- %s\n", unique_crashes, DTD(last_crash_time,start_time), DI(last_crash_execs));
      }
...
/* Check if the result of an execve() during routine fuzzing will crash,
   save or queue the input test case for further analysis if so. Returns 1 if
   entry crashed, 0 otherwise. */

static u8 save_if_crash(char** argv, void* mem, u32 len, u8 fault) {
  ...
  switch (fault) {
    ...
    case FAULT_TMOUT:
      ...
      if (unique_hangs >= KEEP_UNIQUE_HANG) return saved;
      ...
      if (exec_tmout < hang_tmout) {
        ...
        u8 new_fault;
        write_to_testcase(mem, len);
        new_fault = run_target2(argv, hang_tmout);

        /* A corner case that one user reported bumping into: increasing the
           timeout actually uncovers a crash. Make sure we don't discard it if
           so. */

        if (!stop_soon && new_fault == FAULT_CRASH) goto keep_as_crash2;

        if (stop_soon || new_fault != FAULT_TMOUT) return saved;
...
keep_as_crash2:
      ...
      if (unique_crashes >= KEEP_UNIQUE_CRASH) return saved;
      ...
      if (!dumb_mode) {
        ...
        if (!has_new_bits(virgin_crash)) return saved;
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file, 
          "%llu, %llu, %u, %u, %u, %u, %0.02f%%, %llu, %llu, %u, %0.02f, %s\n",
          get_cur_time() / 1000, queue_cycle - 1, current_entry, queued_paths,
          pending_not_fuzzed, pending_favored, bitmap_cvg, unique_crashes,
          unique_hangs, max_depth, eps, DI(total_execs)); /* ignore errors */
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  if (target_path2) {
    ...
    if (is_target) {
        write_to_testcase(out_buf, len);
        fault = run_target2(argv, exec_tmout);
        save_if_crash(argv, out_buf, len, fault);
    }
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  u32 factor = 1;
  if (target_fast && q->is_target) {
    u64 fuzz = q->n_fuzz;
    if (q->fuzz_level < 16) {
      factor = ((u32) (1 << q->fuzz_level)) / (fuzz == 0 ? 1 : fuzz); 
    } else
      factor = 32 / (fuzz == 0 ? 1 : next_p2 (fuzz));
    if (factor > 32) 
        factor = 32;  
  }
...
u8 hit_critical(struct queue_entry* q) {
  u32 i;
  u8 flag = 0;
  ...
  if (!explore_status) {
    for (i = 0; i < q->critical_bbs[0]; i++) {
      if (critical_bits[q->critical_bbs[i+1]] == 1 && !solved_cbbs[q->critical_bbs[i+1]]) {
        u8 cond_id = critical_condition[q->critical_bbs[i+1]];
        if (!cond_id)
          continue;
        if ((condition_bits[2*(cond_id)] != q->critical_vals[2*(i+1)]) || (condition_bits[2*(cond_id)+1] != q->critical_vals[2*(i+1)+1])) {
          flag = 1;
    ...
    for (i = 0; i < q->cond_bits[0]; i++) {
      if (solved_cond[q->cond_bits[i+1]] < 3) {
        ...
        if ((condition_bits[2*id] != q->cond_vals[2*(i+1)]) || (condition_bits[2*id+1] != q->cond_vals[2*(i+1)+1])) {
          flag = 2;
        }
...
bool need_sniff(struct queue_entry* q) {
  if (q->len > 1000)
    return false;
...
static u8 fuzz_one(char** argv) {
  ...
  if ((skip_deterministic && (!target_fast || !queue_cur->is_target)) || queue_cur->was_fuzzed || queue_cur->passed_det)
    goto havoc_stage;
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max; stage_cur++) {
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch (UR(15 + ((extras_cnt + a_extras_cnt) ? 2 : 0))) {
        ...
        case 0:
          ...
          if((posn = get_random_modifiable_posn(1, 1, temp_len, cb_mask, position_map)) == 0xffffffff) 
            posn = UR(temp_len << 3);
        ...
        case 2:
          ...
          if((posn = get_random_modifiable_posn(16, 1, temp_len, cb_mask, position_map)) == 0xffffffff) 
            posn = UR(temp_len - 1);
          if (UR(2)) {
        ...
        case 3:
          ...
          if((posn = get_random_modifiable_posn(32, 1, temp_len, cb_mask, position_map)) == 0xffffffff) 
            posn = UR(temp_len - 3);
          if (UR(2)) {
        ...
        case 11 ... 12: {
            ...
            del_from = get_random_modifiable_posn(del_len*8, 2, temp_len, cb_mask, position_map);
            if (del_from == 0xffffffff) 
              del_from = UR(temp_len - del_len + 1);

            memmove(out_buf + del_from, out_buf + del_from + del_len,
                    temp_len - del_from - del_len);

            memmove(cb_mask + del_from, cb_mask + del_from + del_len,
        ...
        case 13:
          ...
          if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
            ...
            clone_to   = get_random_insert_posn(temp_len, cb_mask, position_map);
            if (clone_to == temp_len) 
              clone_to = UR(temp_len);

            new_buf = ck_alloc_nozero(temp_len + clone_len);
            new_cb_mask = alloc_cb_mask(temp_len + clone_len + 1);

            /* Head */

            memcpy(new_buf, out_buf, clone_to);
            memcpy(new_cb_mask, cb_mask, clone_to);
            ...
            memcpy(new_cb_mask + clone_to + clone_len, cb_mask + clone_to,
                   temp_len - clone_to + 1);
        ...
        case 14: {
            ...
            copy_to   = get_random_modifiable_posn(copy_len * 8, 1, temp_len, cb_mask, position_map);

            if (copy_to == 0xffffffff) 
        ...
        case 15: {
            ...
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
              ...
              insert_at = get_random_modifiable_posn(extra_len * 8, 1, temp_len, cb_mask, position_map);
              if (insert_at == 0xffffffff) 
        ...
        case 16: {
            ...
            u32 use_extra, extra_len, insert_at = get_random_insert_posn(temp_len, cb_mask, position_map);
            ...
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
              ...
              new_cb_mask = alloc_cb_mask(temp_len + extra_len + 1);

              /* Head */
              memcpy(new_buf, out_buf, insert_at);
              memcpy(new_cb_mask, cb_mask, insert_at);
            ...
            memcpy(new_cb_mask + insert_at + extra_len, cb_mask + insert_at,
    ...
    if (temp_len < len) {
      ...
      cb_mask = ck_realloc(cb_mask, len + 1);
    ...
    temp_len = len;
    memcpy(out_buf, in_buf, len);
    memcpy(cb_mask, orig_cb_mask, len + 1);
...
retry_splicing:
  ...
  if (use_splicing && splice_cycle++ < SPLICE_CYCLES &&
    ...
    new_cb_mask = alloc_cb_mask(len + 1);

    memcpy(new_cb_mask, cb_mask, MIN(split_at, temp_len + 1));
    ck_free(cb_mask);
    cb_mask = new_cb_mask;
    ck_free(orig_cb_mask);
    orig_cb_mask = ck_alloc(len +1);
...
static void handle_stop_sig(int sig) {
  ...
  if (child_pid2 > 0) kill(child_pid2, SIGKILL);
  if (forksrv_pid > 0) kill(forksrv_pid, SIGKILL);
  if (forksrv_pid2 > 0) kill(forksrv_pid2, SIGKILL);
}
...
static void handle_timeout(int sig) {
  ...
  } else if (child_pid2 == -1 && forksrv_pid2 > 0) {
    child_timed_out = 1; 
...
EXP_ST void check_binary2(u8* fname) {
  ...
  if (strchr(fname, '/') || !(env_path = getenv("PATH"))) {
    ...
    if (!target_path2) FATAL("Program '%s' not found or not executable", fname);
  ...
  if ((!strncmp(target_path2, "/tmp/", 5) && !strchr(target_path2 + 5, '/')) ||
      (!strncmp(target_path2, "/var/tmp/", 9) && !strchr(target_path2 + 9, '/')))
     FATAL("Please don't keep binaries in /tmp or /var/tmp");

  fd = open(target_path2, O_RDONLY);

  if (fd < 0) PFATAL("Unable to open '%s'", target_path2);

  f_data = mmap(0, f_len, PROT_READ, MAP_PRIVATE, fd, 0);

  if (f_data == MAP_FAILED) PFATAL("Unable to mmap file '%s'", target_path2);
  ...
  if (f_data[0] == '#' && f_data[1] == '!') {
    ...
    FATAL("Program '%s' is a shell script", target_path2);
  ...
  if (f_data[0] != 0xCF || f_data[1] != 0xFA || f_data[2] != 0xED)
    FATAL("Program '%s' is not a 64-bit Mach-O binary", target_path2);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, max_depth, execs_per_sec, total_exec\n");
                     /* ignore errors */
...
void readCondition() {
  ...
  while(fgets(buf, sizeof(buf), condition_file) != NULL) {
    ...
    if (strcmp(token,"none") != 0) {
      ...
      critical_condition[critical_id] = id;
    }

    token = strtok(NULL," "); // the third item
    token = strtok(NULL," "); // the forth item
    if (strcmp(token,"int32") == 0)
      condition_info[id] |= 4;
    if (strcmp(token,"int64") == 0)
      condition_info[id] |= 8;
    token = strtok(NULL," "); // the fifth item
    if (strcmp(token,"var") != 0) {
      // record the value of constant1
      condition_info[id] |= 2;
      u64 value = strtoul(token,0,10);
      condition_values[2*id] = value;
    }
    token = strtok(NULL," "); // the sixth item
    if (strcmp(token,"var") != 0) {
      // record the value of constant2
      condition_info[id] |= 1;
      u64 value = strtoul(token,0,10);
      condition_values[2*id+1] = value;
    }
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:E:dnCB:S:M:x:Qz:c:ealkjpus")) > 0)
    ...
    switch (opt) {
      ...
      case 'e':
        if (no_critical_flag) FATAL("Multiple -e options not supported");
        no_critical_flag = 1;
        break;

      case 'a':
        if (no_distance_favor_flag) FATAL("Multiple -a options not supported");
        no_distance_favor_flag = 1;
        break;

      case 'l':
        if (afl_flag) FATAL("Multiple -l options not supported");
        afl_flag = 1;
        break;

      case 'k':
        if (no_cb_mutation_flag) FATAL("Multiple -k options not supported");
        no_cb_mutation_flag = 1;
        break;

      case 'E':
        if (target_path2) FATAL("Multiple -E options not supported");
        target_path2 = optarg;
        break;

      case 'j':
        if (conformance_flag) FATAL("Multiple -j options not supported");
        conformance_flag = 1;
        break;

      case 'p':
        if (no_target_favor) FATAL("Multiple -p options not supported");
        no_target_favor = 1;
        break;

      case 'u':
        if (target_fast) FATAL("Multiple -u options not supported");
        target_fast = 1;
        break;

      case 's':
        if (no_dynamic_switch) FATAL("Multiple -s options not supported");
        no_dynamic_switch = 1;
        break;
  ...
  while (1) {
    ...
    if (!no_critical_flag) {
      if (!no_dynamic_switch) {
        for (i=0;i<critical_ids[0];i++) {
          if (critical_count[critical_ids[i+1]] && !solved_cbbs[critical_ids[i+1]] && critical_count[critical_ids[i+1]] < BLOCK_TIMES) {
            explore_status = 0;
          }
        }
      }
